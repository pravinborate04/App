...
public class ResolutionsFragment extends Fragment implements DeviceFragment.DeviceTypeAndDateRangeListener {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        pieChart = (PieChart) view.findViewById(R.id.piechartResolution);
        piechart1=(PieChart)view.findViewById(R.id.piechartResolution1);
        pieChart.setRotationEnabled(false);
        piechart1.setRotationEnabled(false);
    }


    public void setPieChart(List<ResolutionResponseModel> pieChartModels) {
        ...
        for(int i=0;i<pieChartModels.size();i++){
            ...
            xVals.add(pieChartModels.get(i).getResolution());
        }
    ...
    public void deviceTypeChange(String sd, String ed, String type) {
        Log.e("Resolution","sd "+sd+" ed "+ed+" type "+type);
        Tools.showProgress(getActivity());
        webServices= RetrofitManager.getInstance().call();
        webServices.getDeviceCounters(Constants.SEARCH_BY_RESOLUTION,sd,ed,type, App.getInstance().getLoginUser().getAkey()).enqueue(new Callback<String>() {
            @Override
            public void onResponse(Call<String> call, Response<String> response) {
                if(response.body()!=null) {
                    Log.e("resp resoln", response.body());
                    Tools.hideProgress();
                    List<ResolutionResponseModel> resolutionResponseModels = new ArrayList<ResolutionResponseModel>();
                    try {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            ResolutionResponseModel resolutionResponseModel;
                            resolutionResponseModel = new Gson().fromJson(jsonArray.getString(i), ResolutionResponseModel.class);
                            resolutionResponseModels.add(resolutionResponseModel);
                        }
                    ...
                    Log.e("Act size", resolutionResponseModels.size() + "");
                    setPieChart(resolutionResponseModels);
                }
            }
