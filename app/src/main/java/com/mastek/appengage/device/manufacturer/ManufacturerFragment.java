...
public class ManufacturerFragment extends Fragment implements DeviceFragment.DeviceTypeAndDateRangeListener
{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_manufacturer, container, false);
    }
    ...
    public void setPieChart(List<ManufacturerResponseModel> pieChartModels) {
        ...
        for(int i=0;i<pieChartModels.size();i++){
            ...
            xVals.add(pieChartModels.get(i).getManufacturer());
        }
    ...
    public void deviceTypeChange(String sd, String ed, String type) {
        Log.e("Manufacturer","sd "+sd+" ed "+ed+" type "+type);

        webServices= RetrofitManager.getInstance().call();
        Tools.showProgress(getActivity());
        webServices.getDeviceCounters(Constants.SEARCH_BY_MANUFACTURER,sd,ed,type, App.getInstance().getLoginUser().getAkey()).enqueue(new Callback<String>() {
            @Override
            public void onResponse(Call<String> call, Response<String> response) {
                if(response.body()!=null) {
                    Log.e("resp", response.body());
                    Tools.hideProgress();
                    List<ManufacturerResponseModel> manufacturerResponseModelList = new ArrayList<ManufacturerResponseModel>();
                    try {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            ManufacturerResponseModel manufacturerResponseModel;
                            manufacturerResponseModel = new Gson().fromJson(jsonArray.getString(i), ManufacturerResponseModel.class);
                            manufacturerResponseModelList.add(manufacturerResponseModel);
                        }
                    ...
                    Log.e("Act size", manufacturerResponseModelList.size() + "");
                    setPieChart(manufacturerResponseModelList);
                }
            }
