...
public class TypeFragment extends Fragment implements DeviceFragment.DeviceTypeAndDateRangeListener {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        pieChart = (PieChart) view.findViewById(R.id.piechartType);
        piechart1 = (PieChart) view.findViewById(R.id.piechartType1);
        pieChart.setRotationEnabled(false);
        piechart1.setRotationEnabled(false);
    }


    public void setPieChart(List<TypeResponseModel> pieChartModels) {
        ...
        for (int i = 0; i < pieChartModels.size(); i++) {
            ...
            xVals.add(pieChartModels.get(i).getType());
        }
    ...
    public void deviceTypeChange(String sd, String ed, String type) {
        Log.e("Type", "sd " + sd + " ed " + ed + " type " + type);

        webServices = RetrofitManager.getInstance().call();
        webServices.getDeviceCounters(Constants.SEARCH_BY_TYPE, sd, ed, type, App.getInstance().getLoginUser().getAkey()).enqueue(new Callback<String>() {
            @Override
            public void onResponse(Call<String> call, Response<String> response) {

                if (response.body() != null) {
                    Log.e("resp", response.body());
                    List<TypeResponseModel> typeResponseModelList = new ArrayList<TypeResponseModel>();
                    try {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            TypeResponseModel typeResponseModel;
                            typeResponseModel = new Gson().fromJson(jsonArray.getString(i), TypeResponseModel.class);
                            typeResponseModelList.add(typeResponseModel);
                        }
                    ...
                    Log.e("Act size", typeResponseModelList.size() + "");
                    setPieChart(typeResponseModelList);
                }
            }
