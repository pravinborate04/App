...
public class AppVersionsFragment extends Fragment implements DeviceFragment.DeviceTypeAndDateRangeListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_app_versions, container, false);
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        pieChart = (PieChart) view.findViewById(R.id.piechartAppVersion);
        piechart1=(PieChart)view.findViewById(R.id.piechartAppVersion1);
        pieChart.setRotationEnabled(false);
        piechart1.setRotationEnabled(false);
    }


    public void setPieChart(List<AppVersionResposeModel> pieChartModels) {
        ...
        for(int i=0;i<pieChartModels.size();i++){
            ...
            xVals.add(pieChartModels.get(i).getAppversion());
        }
    ...
    public void deviceTypeChange(String sd, String ed, String type) {
        Log.e("AppVersions","sd "+sd+" ed "+ed+" type "+type);


        webServices= RetrofitManager.getInstance().call();
        Tools.showProgress(getActivity());
        webServices.getDeviceCounters(Constants.SEARCH_BY_APP_VERSION,sd,ed,type, App.getInstance().getLoginUser().getAkey()).enqueue(new Callback<String>() {
            @Override
            public void onResponse(Call<String> call, Response<String> response) {

                if(response.body()!=null) {
                    Log.e("resp App", response.body());
                    Tools.hideProgress();
                    List<AppVersionResposeModel> appVersionResposeModelArrayList = new ArrayList<AppVersionResposeModel>();
                    try {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            AppVersionResposeModel appVersionResposeModel;
                            appVersionResposeModel = new Gson().fromJson(jsonArray.getString(i), AppVersionResposeModel.class);
                            appVersionResposeModelArrayList.add(appVersionResposeModel);
                        }
                    ...
                    Log.e("Act size", appVersionResposeModelArrayList.size() + "");
                    setPieChart(appVersionResposeModelArrayList);
                }
