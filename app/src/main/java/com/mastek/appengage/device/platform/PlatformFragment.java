...
public class PlatformFragment extends Fragment implements DeviceFragment.DeviceTypeAndDateRangeListener {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        pieChart = (PieChart) view.findViewById(R.id.piechartPlatform);
        piechart1=(PieChart)view.findViewById(R.id.piechartPlatform1);
        pieChart.setRotationEnabled(false);
        piechart1.setRotationEnabled(false);
    }


    public void setPieChart(List<PlatformResponseModel> pieChartModels) {
        ...
        for(int i=0;i<pieChartModels.size();i++){
            ...
            xVals.add(pieChartModels.get(i).getPlatform());
        }
    ...
    public void deviceTypeChange(String sd, String ed, String type) {
        Log.e("Platform","sd "+sd+" ed "+ed+" type "+type);
        Tools.showProgress(getActivity());
        webServices= RetrofitManager.getInstance().call();
        webServices.getDeviceCounters(Constants.SEARCH_BY_PLATFORM,sd,ed,type, App.getInstance().getLoginUser().getAkey()).enqueue(new Callback<String>() {
            @Override
            public void onResponse(Call<String> call, Response<String> response) {

                if(response.body()!=null) {
                    Log.e("resp plat", response.body());
                    Tools.hideProgress();
                    List<PlatformResponseModel> platformResponseModelList = new ArrayList<PlatformResponseModel>();
                    try {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            PlatformResponseModel platformResponseModel;
                            platformResponseModel = new Gson().fromJson(jsonArray.getString(i), PlatformResponseModel.class);
                            platformResponseModelList.add(platformResponseModel);
                        }
                    ...
                    Log.e("Act size", platformResponseModelList.size() + "");
                    setPieChart(platformResponseModelList);
                }
            }
